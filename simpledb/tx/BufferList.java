...
class BufferList {
    ...
    * @param blk a reference to the disk block
    */
   void pin(Block blk, int txnum) {
      Buffer buff = bufferMgr.pin(blk, txnum);
    ...
    */
   Block pinNew(String filename, PageFormatter fmtr, int txnum) {
      Buffer buff = bufferMgr.pinNew(filename, fmtr, txnum);
    ...
    * @param blk a reference to the disk block
    */
   void unpin(Block blk, int txnum) {
      Buffer buff = buffers.get(blk);
      bufferMgr.unpin(buff, txnum);
      pins.remove(blk);
